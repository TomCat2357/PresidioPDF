...
log_filename = os.path.join(
    LOG_FOLDER, f"presidio_web_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
)
logging.basicConfig(
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="PDF個人情報マスキングツール - Webアプリケーション版"
    )
    parser.add_argument(
        "--gpu",
        action="store_true",
        help="GPU（NVIDIA CUDA）を使用する（デフォルト: CPU使用）",
    )
    parser.add_argument(
        "--host",
        default="0.0.0.0",
        help="サーバーのホストアドレス（デフォルト: 0.0.0.0）",
    )
    parser.add_argument(
        "--port",
        type=int,
