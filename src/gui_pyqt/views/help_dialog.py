...
<h3>基本フロー</h3>
<ol>
  <li>「ファイル」▾ の <b>開く</b> で PDF を読み込みます。</li>
  <li>「処理」▾ の <b>対象検出</b> または <b>OCR</b> を必要に応じて実行します。</li>
  <li>結果表で <b>削除</b>、<b>無視対象</b>、<b>追加検出対象</b> を使って調整します。</li>
  <li>「処理」▾ の <b>重複削除</b> で候補を整理します。</li>
  <li>「ファイル」▾ の <b>保存</b> または <b>エクスポート</b> で出力します。</li>
</ol>
<h3>主要ショートカット</h3>
...
""",
    ...
    "file": {
        ...
        "body": """
<h3>対象</h3>
<p>「ファイル」▾ には <b>開く</b>・<b>閉じる</b>・<b>保存</b>・<b>エクスポート</b>（サブメニュー）があります。
クリックするとメニューが開きます。<b>開く</b> はショートカット <code>Ctrl+O</code> でも直接実行できます。</p>
<h3>開く・閉じる</h3>
<ul>
  <li><b>開く</b> ではマッピング情報があれば自動で読み込みます。ショートカットは <code>Ctrl+O</code> です。</li>
  <li><b>閉じる</b> や別ファイル切り替え時は、未保存変更があると確認ダイアログが出ます。</li>
  <li>PDF はドラッグ＆ドロップでも開けます。</li>
</ul>
<h3>保存とエクスポートの違い</h3>
<ul>
  <li><b>保存</b>（<code>Ctrl+S</code>）は現在の PDF とサイドカーJSONを更新し、作業状態を引き継げるようにします。</li>
  <li><b>エクスポート</b> は配布・確認用の別出力（アノテーション付き／マスク／セキュアマスク保存／マーク画像／検出結果一覧CSV）を作成します。</li>
</ul>
<p>作業の続きが必要なときはまず保存、成果物を出したいときはエクスポートを使います。検出・重複削除の設定は「設定」▾で行います。</p>
""",
    },
    "settings": {
        "title": "設定",
...
""",
    ...
    "processing": {
        ...
        "body": """
<h3>対象</h3>
<p>「処理」▾ には <b>対象検出</b>・<b>対象削除</b>・<b>重複削除</b> のサブメニューがあります。</p>
<h3>対象検出</h3>
<p>PII 候補を抽出します。設定ダイアログの検出対象や前処理設定が結果に反映されます。</p>
<ul>
  <li><b>表示ページ</b>: 今見ているページだけを検出します。ショートカットは <code>F5</code> です。</li>
  <li><b>全ページ</b>: すべてのページを検出します。ショートカットは <code>Shift+F5</code> です。</li>
</ul>
<h3>対象削除</h3>
<p>自動検出された候補だけをまとめて削除します。手動追加や追加検出対象から来た候補は削除対象にしません。</p>
<ul>
  <li><b>表示ページ</b> / <b>全ページ</b>: 削除範囲を選べます。</li>
</ul>
<h3>重複削除</h3>
<p>重なった検出結果を優先順位で整理します。優先順位の詳細はヘルプ▾の「重複削除優先順位」を参照してください。</p>
<ul>
  <li><b>表示ページ</b> / <b>全ページ</b>: 処理範囲を選べます。</li>
</ul>
""",
    },
    "ocr": {
