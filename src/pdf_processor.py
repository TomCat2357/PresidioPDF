...
class PDFProcessor:
    ...
    def analyze_pdf(self, pdf_path: str) -> List[Dict]:
        ...
        for result in results:
            ...
            if result['line_rects']:
                ...
                result['coordinates'] = {'page_number': page_num + 1, 'x0': float(main_rect.x0), 'y0': float(main_rect.y0), 'x1': float(main_rect.x1), 'y1': float(main_rect.y1)}
        ...
        return sorted(results, key=lambda x: (x.get('page_info', {}).get('page_number', 0), x.get('coordinates', {}).get('y0', 0)))
    ...
    def process_pdf_file(self, input_path: str, masking_method: str = None) -> Dict:
        ...
        try:
            ...
            output_path = self.masker.apply_masking(input_path, entities, masking_method)
            logger.info(f"PDF処理完了: {output_path} ({len(entities)}件の個人情報を処理)")
            
            summary = self._create_summary(input_path, output_path, backup_path, entities)
    ...
    def _read_pdf_file(self, pdf_path: str) -> Dict:
        ...
        try:
            ...
            if self.config_manager.should_generate_read_report():
                report_file = self.annotator.generate_annotations_report(annotations, pdf_path)
            
            logger.info(f"PDF注釈読み取り完了: {pdf_path} ({len(annotations)}件の注釈)")
            return {
                'input_file': pdf_path, 'total_annotations': len(annotations),
                'annotations': annotations, 'report_file': report_file
            }
        except Exception as e:
    ...
    def _update_stats(self, summary: Dict):
        ...
        for e_type, count in summary['entities_by_type'].items():
            self.processing_stats['entities_by_type'][e_type] = self.processing_stats['entities_by_type'].get(e_type, 0) + count
    ...
    def _generate_report(self, results: List[Dict]):
        ...
        try:
            if fmt == 'json':
                report_data = {'processing_stats': self.processing_stats, 'file_results': results}
                ...
                with open(report_filename, 'w', newline='', encoding='utf-8') as f:
                    ...
                    for res in results:
                        if 'error' not in res and 'skipped' not in res:
                            types = ', '.join(f"{k}:{v}" for k, v in res.get('entities_by_type', {}).items())
                            writer.writerow([res['input_file'], res['total_entities_found'], 'Success', types])
                        else:
