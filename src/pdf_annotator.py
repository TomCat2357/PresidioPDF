...
class PDFAnnotator:
        ...
        try:
            ...
            for annotation_data in annotations:
                try:
                    ...
                    logger.warning(f"注釈復元エラー: {e} (注釈: {annotation_data.get('annotation_type', 'Unknown')})")
    ...
    def _restore_single_annotation(self, doc: fitz.Document, annotation_data: Dict) -> bool:
        ...
        try:
            ...
            if annotation_type == "Highlight":
                return self._restore_highlight_from_data(page, annotation_data, coordinates)
            else:
                return self._restore_annotation_from_data(page, annotation_data, coordinates)
    ...
    def _extract_covered_text(self, rect: fitz.Rect, page) -> str:
        ...
        try:
            ...
            return ' '.join([word[4] for word in text_instances]) if text_instances else ""
    ...
    def _extract_annotation_colors(self, annot) -> Dict:
        ...
        try:
            ...
            if annot.colors.get('stroke'):
                ...
                colors['stroke_color'] = {'rgb': list(stroke), 'hex': self._rgb_to_hex(stroke)}
            if annot.colors.get('fill'):
                fill = annot.colors['fill']
                colors['fill_color'] = {'rgb': list(fill), 'hex': self._rgb_to_hex(fill)}
            return colors
        except Exception as e:
    ...
    def _restore_highlight_from_data(self, page: fitz.Page, data: Dict, coords: Dict) -> bool:
        """データからハイライトを復元"""
    ...
    def _restore_annotation_from_data(self, page: fitz.Page, data: Dict, coords: Dict) -> bool:
        """データから注釈を復元"""
        rect = self._extract_rect_from_coordinates(coords)
        if not rect: return False
    ...
    def _extract_rect_from_coordinates(self, coords: Dict) -> Optional[fitz.Rect]:
        ...
        try:
            x0, y0, x1, y1 = float(coords['x0']), float(coords['y0']), float(coords['x1']), float(coords['y1'])
    ...
    def _extract_color_from_report(self, color_info: Dict, color_type: str) -> Optional[List[float]]:
        """レポートの色情報からRGBリストを抽出"""
        try:
