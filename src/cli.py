...
@click.option('--read-report/--no-read-report', default=True, help='読み取りレポートを生成 (デフォルト: True)')
...
@click.option('--masking-method', type=click.Choice(['annotation', 'highlight', 'both']), help='マスキング方式')
@click.option('--masking-text-mode', type=click.Choice(['silent', 'minimal', 'verbose']), help='マスキング文字表示モード')
@click.option('--operation-mode', type=click.Choice(['clear_all', 'append', 'reset_and_append']), help='操作モード')
@click.option('--spacy-model', '-m', type=str, help='使用するspaCyモデル名')
@click.option('--deduplication-mode', type=click.Choice(['wider_range', 'narrower_range', 'entity_type']), help='重複除去モード')
@click.option('--deduplication-overlap-mode', type=click.Choice(['contain_only', 'partial_overlap']), help='重複判定モード')
def main(path, **kwargs):
    ...
    args_dict['pdf_masking_method'] = args_dict.pop('masking_method', None) # key rename
    
    config_manager = ConfigManager(config_file=args_dict.get('config'), args=args_dict)
    
    if config_manager._safe_get_config('features.logging.level') == 'DEBUG' or args_dict.get('verbose'):
    ...
    try:
        if args_dict.get('restore_mode'):
            ...
            output_path = processor.masker._generate_output_path(path) # Use masker's helper
...
def display_read_results(results):
    ...
    total_annotations = sum(r.get('total_annotations', 0) for r in results if 'error' not in r)
...
def display_masking_results(results, config_manager):
    ...
    total_entities = sum(r.get('total_entities_found', 0) for r in results if 'error' not in r)
