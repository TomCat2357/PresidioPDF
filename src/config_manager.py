...
class ConfigManager:
    ...
    # サポートされているエンティティタイプ
    ENTITY_TYPES = ["PERSON", "LOCATION", "DATE_TIME", "PHONE_NUMBER", 
                   "INDIVIDUAL_NUMBER", "YEAR", "PROPER_NOUN"]
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "custom_recognizers": {},
            "features": {
                "logging": {"level": "INFO", "log_to_file": False, "log_file_path": "presidio_log.txt"},
                "backup": {"create_backup": False, "backup_suffix": "_backup"},
                "reporting": {"generate_report": False, "report_format": "json", "report_file_prefix": "presidio_report", "include_detected_text": False},
                "processing": {"batch_size": 100, "parallel_processing": False, "skip_processed_files": True},
                "file_handling": {"supported_formats": [".pdf"], "output_suffix": "_highlighted", "recursive_search": True, "output_dir": None}
            },
            "nlp": {
    ...
    def _convert_args_to_config(self, args: Dict) -> Dict[str, Any]:
        ...
        if 'suffix' in args and args['suffix']:
            ...
            config['features']['file_handling'] = config['features'].get('file_handling', {})
    ...
    def get_custom_recognizers(self) -> Dict[str, Dict]:
        """カスタム認識器の設定を返す"""
        return {k: v for k, v in self.config['custom_recognizers'].items() 
    ...
    def get_output_suffix(self) -> str:
        """出力ファイルのサフィックスを返す"""
        return self._safe_get_config('features.file_handling.output_suffix', '_highlighted')
    ...
    def get_entity_exclusions(self, entity_type: str = None) -> Union[List[str], Dict[str, List[str]]]:
        """エンティティ除外リストを返す"""
    ...
    def is_entity_excluded(self, entity_type: str, text: str) -> bool:
        ...
        for exclusion in text_exclusions:
            if text in exclusion:
                logger.debug(f"共通除外ワードにより除外: '{text}' が '{exclusion}' に含まれています")
                return True
    ...
    def get_pdf_annotation_settings(self) -> Dict[str, Any]:
        """PDF注釈設定を返す"""
        return self._safe_get_config('pdf_processing.masking.annotation_settings', {
            'include_text': False,
            'font_size': 12,
            'font_family': 'Helvetica'
        })
    ...
    def get_pdf_file_exclusions(self) -> List[str]:
        """PDF処理のファイル除外パターンを返す"""
        return self._safe_get_config('pdf_processing.exclusions.files', ['*_backup.*', '*_masked.*'])
    ...
    def should_include_detected_text_in_pdf_report(self) -> bool:
        ...
        return self._safe_get_config('pdf_processing.report.include_detected_text', False)
    ...
    def is_pdf_parallel_processing_enabled(self) -> bool:
        """PDF並列処理が有効かどうかを返す"""
        return self._safe_get_config('pdf_processing.processing.parallel_processing', False)
    
    def should_skip_processed_pdf_files(self) -> bool:
        """処理済みPDFファイルをスキップするかどうかを返す"""
        return self._safe_get_config('pdf_processing.processing.skip_processed_files', True)
    ...
    def get_entity_priority_order(self) -> List[str]:
        """エンティティタイプの優先順序を返す"""
        return self._safe_get_config('deduplication.entity_priority_order', 
                                    ["INDIVIDUAL_NUMBER", "PHONE_NUMBER", "PERSON", "LOCATION", "DATE_TIME", "YEAR", "PROPER_NOUN"])
