...
class PresidioPDFGUITkEasy:
    """PDF個人情報マスキングツールのTkEasyGUI版"""
    ...
    def _create_main_window(self):
        ...
        properties_layout = [
            ...
            [sg.Text('タイプ:', size=(8, 1)), 
             sg.Combo(['PERSON', 'LOCATION', 'PHONE_NUMBER', 'DATE_TIME'], 
                      key='-ENTITY_TYPE-', size=(20, 1), readonly=True)],
        ...
        # メインレイアウト
        layout = [
            [sg.Frame('ファイル操作', file_layout, font=('Arial', 12))],
            [sg.HSeparator()],
            [sg.Frame('検出結果', results_layout, font=('Arial', 12))],
            [sg.HSeparator()],
            [sg.Frame('プロパティ', properties_layout, font=('Arial', 12))],
            [sg.HSeparator()],
            [sg.Text('', key='-STATUS-', size=(60, 1)),
             sg.Text('', key='-PROGRESS-', size=(20, 1), visible=False),
    ...
    def _show_progress(self, visible: bool = True):
        ...
        if self.window:
            if visible:
                self.window['-PROGRESS-'].update('処理中...', visible=True)
            else:
    ...
    def _run_detection(self):
        ...
        try:
            ...
            # 実際のPresidio処理を試行し、失敗したら模擬データを使用
            try:
    ...
    def _extract_detection_results_from_report(self, report_data: dict):
        ...
        if 'annotations' in report_data:
            for annotation in report_data['annotations']:
                ...
                # コンテンツからエンティティ情報を推測
                entity_type, confidence, text = self._extract_entity_from_content(content)
    ...
    def _extract_entity_from_content(self, content: str):
    ...
    def _select_entity(self, index: int):
        ...
        if 0 <= index < len(self.detection_results):
            ...
            # プロパティパネル更新
            self.window['-PROP_STATUS-'].update("選択されたエンティティ")
            self.window['-ENTITY_TYPE-'].update(value=entity["entity_type"])
    ...
    def _clear_properties_panel(self):
        """プロパティパネルをクリア"""
        self.window['-PROP_STATUS-'].update("エンティティが選択されていません")
        self.window['-ENTITY_TYPE-'].update(value="")
    ...
    def _open_settings_dialog(self):
        ...
        settings_window = sg.Window('設定', settings_layout, modal=True)
        ...
        while True:
            ...
            # リスト選択
                ...
                if values['-RESULTS-']:
                    ...
                    if selected_items:
                        # 選択された項目のインデックスを取得
                        all_items = self.window['-RESULTS-'].Values
                        selected_index = all_items.index(selected_items[0])
                        self._select_entity(selected_index)
            
            # アノテーション操作
            elif event == '-DELETE-':
                if self.selected_index >= 0:
                    self._delete_annotation(self.selected_index)
