            ...
            try:
                if page.locator(selector).count() > 0:
                    ...
                    if element.is_visible():
                        spinner_found = True
                        print(f"ローディングスピナー検出: {selector} (チェック {check + 1}/{max_checks})")
                        break
            except:
                continue
        
        if not spinner_found:
            print(f"ローディングスピナーが検出されませんでした (チェック {check + 1}/{max_checks})")
            # 追加で安全な待機時間を設ける
            time.sleep(3)
            return True
...
def take_screenshot_with_retry(page, path, description, max_retries=5):
    ...
    for retry in range(max_retries):
        ...
        if not loading_complete:
            print(f"ローディング完了を待機できませんでした (試行 {retry + 1}/{max_retries})")
            time.sleep(5)
            continue
...
def run_web_app_test():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # ページのスクリーンショット
            take_screenshot_with_retry(page, "/workspace/outputs/01_initial_page.png", "初期ページのスクリーンショット")
            ...
            time.sleep(2)
            take_screenshot_with_retry(page, "/workspace/outputs/02_after_upload.png", "ファイルアップロード後のスクリーンショット")
            ...
            try:
                # 結果が表示されるまで待機
                page.wait_for_selector(".results-container, .detection-results, #results", timeout=60000)
            ...
            take_screenshot_with_retry(page, "/workspace/outputs/03_after_detection.png", "検出処理完了後のスクリーンショット")
            ...
            # 結果をファイルに保存
            with open("/workspace/outputs/detection_results_text.txt", "w", encoding="utf-8") as f:
                f.write(page_text)
            ...
            page.screenshot(path="/workspace/outputs/04_full_page_with_highlights.png", full_page=True)
            ...
            if pdf_viewer.count() > 0:
                wait_for_loading_complete(page)
                pdf_viewer.first.screenshot(path="/workspace/outputs/05_pdf_viewer_highlights.png")
